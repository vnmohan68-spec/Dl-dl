...
# ══════════════════════════════════════════════════════════════════════════════
CONFIDENCE_THRESH = 0.60   # top-1 must beat 60%
CONFIDENCE_GAP    = 0.10   # top1 − top2 gap must exceed 10%
TEMP_SCALE        = 0.80   # softmax temperature
BLUR_THRESHOLD    = 30.0   # Laplacian variance; below = blurry
BRIGHTNESS_MIN    = 20     # mean brightness; below = too dark
BRIGHTNESS_MAX    = 245    # mean brightness; above = overexposed

VALID_PLANTS       = ["pepper", "potato", "tomato"]
UNKNOWN_CLASS_NAME = "Unknown_Not_A_Plant"

# ── Claude Vision API — gatekeeper before disease model runs ──────────────────
...
def check_blur(pil_image: Image.Image) -> bool:
    ...
    lap = (windows * kern).sum(axis=(-1, -2))
    return float(lap.var()) >= BLUR_THRESHOLD
...
# ══════════════════════════════════════════════════════════════════════════════
#  CLAUDE VISION GATEKEEPER
#  Uses Claude AI to look at the image and decide:
#  — Is this a tomato / potato / bell pepper leaf?
#  — Or is it a face, money plant, neem, coconut, random object, etc.?
#  This is 100x more accurate than any pixel/color heuristic.
# ══════════════════════════════════════════════════════════════════════════════

def claude_vision_gate(pil_image: Image.Image) -> dict:
    """
    Sends the image to Claude Vision API.
    Returns: {"allowed": True/False, "reason": str, "detected": str}
    """
    import urllib.request, json as _json

    # Resize to reduce token cost — 512px is enough for Claude to understand content
    img_small = pil_image.convert("RGB")
    img_small.thumbnail((512, 512), Image.LANCZOS)
    buf = BytesIO()
    img_small.save(buf, format="JPEG", quality=85)
    b64_data = base64.b64encode(buf.getvalue()).decode()

    prompt = """Look at this image carefully. Answer ONLY with a JSON object, no extra text.

Rules:
- "allowed": true ONLY if the image shows a CLEAR, CLOSE-UP LEAF of tomato, potato, or bell pepper plant.
- "allowed": false for EVERYTHING else — including:
  * Human faces, selfies, people, body parts
  * Money plant, coconut leaves, neem, banana leaves, or ANY other plant not listed
  * Flowers, fruits, roots, stems, bark, soil
  * Animals, objects, food, buildings, sky, roads
  * Blurry or distant shots where leaf species cannot be confirmed
  * Multiple plants mixed together

Respond ONLY with this exact JSON:
{"allowed": true/false, "detected": "what you see in 5 words", "reason": "one sentence why"}"""
    ...
    try:
        ...
        return {
            ...
            "reason":   result.get("reason", "Could not determine image content.")
        }
    except Exception as e:
        # If Vision API fails, fall back to allowing (don't block real leaves)
        return {"allowed": True, "detected": "unknown", "reason": f"Vision check skipped: {e}"}


def smart_predict(pil_image: Image.Image, model, class_names: list) -> dict:
    """
    Guard pipeline:
      1. Brightness / blur check (image quality)
      2. Claude Vision gatekeeper (is this a tomato/potato/pepper leaf?)
      3. Disease model inference
      4. Confidence + class whitelist checks
    """
    def reject(reason):
    ...
    # Guard 3 — Claude Vision gatekeeper (THE KEY GUARD)
    vision = claude_vision_gate(pil_image)
    if not vision["allowed"]:
        detected = vision["detected"]
        return reject(
            f"Detected: {detected}. "
            f"{vision['reason']} "
            "Please upload only a clear, close-up leaf photo of Tomato, Potato, or Bell Pepper."
        )

    # Guard 4 — Disease model inference
    img_arr = np.array(
    ...
    # Guard 5 — Model predicted Unknown class
    if UNKNOWN_CLASS_NAME.lower() in top_label.lower():
        return reject(
            "The disease model could not identify this as a supported plant leaf. "
            "Upload only Tomato, Potato or Bell Pepper leaf images.")

    # Guard 6 — Class whitelist
...
with col_right:
    if uploaded is None:
        ...
        if model is None:
            ...
            with st.spinner("🔬 Scanning image — verifying leaf type, then running disease analysis…"):
